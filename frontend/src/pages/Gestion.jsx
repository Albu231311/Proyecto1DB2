...
export default function Gestion() {
  ...
  const handleAddNewClick = () => {
    ...
    if (tab === "usuarios") {
      ...
      setFormDataOrden({ usuarioId: "", restauranteId: "", items: [{ productoId: "", cantidad: 1 }] });
    } else {
  ...
  const addField = (type) => {
    if (tab === "restaurantes") {
      ...
      setFormDataOrden({...formDataOrden, items: [...formDataOrden.items, { productoId: "", cantidad: 1 }]});
    }
  };
  ...
  const removeField = (index, type) => {
    if (tab === "restaurantes") {
      ...
      const filtered = formDataOrden.items.filter((_, i) => i !== index);
      setFormDataOrden({...formDataOrden, items: filtered});
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = `/${tab}`;
      let data;
      if (tab === "usuarios") data = formDataUsuario;
      else if (tab === "menu") data = formDataMenu;
      else if (tab === "resenas") data = formDataResena;
      else if (tab === "ordenes") data = formDataOrden;
      else data = formDataRestaurante;
            ...
            <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => addField('dir_res')}>+ Añadir</button>
                </div>
                {formDataRestaurante.direcciones.map((dir, i) => (
                  <div key={i} style={{ padding: '12px', background: 'var(--slate-50)', borderRadius: '8px', marginBottom: '8px', border: '1px solid var(--slate-200)', position: 'relative' }}>
                    <button type="button" onClick={() => removeField(i, 'dir_res')} style={{ position: 'absolute', right: 8, top: 8, color: 'var(--red-500)', background: 'none', border: 'none' }}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>delete</span></button>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => addField('dir_usr')}>+ Añadir</button>
                </div>
                {formDataUsuario.direccionesEnvio.map((dir, i) => (
                  <div key={i} style={{ padding: '12px', background: 'var(--slate-50)', borderRadius: '8px', marginBottom: '8px', border: '1px solid var(--slate-200)', position: 'relative' }}>
                    <button type="button" onClick={() => removeField(i, 'dir_usr')} style={{ position: 'absolute', right: 8, top: 8, color: 'var(--red-500)', background: 'none', border: 'none' }}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>delete</span></button>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 600, width: '95%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--slate-500)', marginBottom: 4 }}>ID Usuario</label>
                  <input required placeholder="ID del Usuario" className="input-field" value={formDataOrden.usuarioId} onChange={e => setFormDataOrden({...formDataOrden, usuarioId: e.target.value})} style={{ width: '100%', padding: '8px', borderRadius: 6, border: '1px solid var(--slate-200)' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--slate-500)', marginBottom: 4 }}>ID Restaurante</label>
